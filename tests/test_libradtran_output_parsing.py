...
"""
    ...
    def test_integrated_single_altitude_parsing(self, minimal_config):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.out', delete=False) as tmp:
            output_file = self.create_mock_libradtran_output("integrated_single_altitude", tmp.name)
    ...
    def test_integrated_multi_altitude_parsing(self, minimal_config):
        ...
        # Create mock output file
        with tempfile.NamedTemporaryFile(mode='w', suffix='.out', delete=False) as tmp:
            output_file = self.create_mock_libradtran_output("integrated_multi_altitude", tmp.name)
    ...
    def test_spectral_single_altitude_parsing(self, minimal_config):
        ...
        # Create mock output file
        with tempfile.NamedTemporaryFile(mode='w', suffix='.out', delete=False) as tmp:
            output_file = self.create_mock_libradtran_output("spectral_single_altitude", tmp.name)
    ...
    def test_spectral_multi_altitude_parsing(self, minimal_config):
        ...
        # Create mock output file
        with tempfile.NamedTemporaryFile(mode='w', suffix='.out', delete=False) as tmp:
            output_file = self.create_mock_libradtran_output("spectral_multi_altitude", tmp.name)
        ...
        try:
            ...
            assert result.data['eglo'][0.0][400.0] == 750.5  # First altitude, first wavelength
            assert result.data['eglo'][1.0][400.0] == 720.5  # Second altitude, first wavelength
            assert result.data['eglo'][2.0][400.0] == 690.5  # Third altitude, first wavelength
