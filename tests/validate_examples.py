...
from pyradtran.config import load_config, SimulationConfig
...
def resolve_paths(params: Dict[str, Any], example_dir: Path, libradtran_data: Path) -> Dict[str, Any]:
    ...
    path_keys = [
        'atmosphere_file', 'solar_file', 'source', 'mol_tau_file', 
        'aerosol_default', 'cloud_overlap_file', 'slit_function_file'
    ]
...
def compare_outputs(generated_file: Path, reference_file: Path) -> bool:
    ...
    try:
        ...
        if gen_data.shape != ref_data.shape:
            logger.error(f"Shape mismatch: Generated {gen_data.shape} vs Reference {ref_data.shape}")
            return False
...
def create_yaml_config(params: Dict[str, Any], output_yaml_path: Path):
    ...
    if 'mol_abs_param' in params:
        yaml_config['simulation_defaults']['mol_abs_param'] = params.pop('mol_abs_param')
...
def main():
    ...
    parser.add_argument("--limit", type=int, default=0, help="Limit number of examples to run")
