...
from pyradtran.config import load_config, SimulationConfig, SimulationDefaults, CloudParameters
...
def resolve_paths(params: Dict[str, Any], example_dir: Path, libradtran_data: Path) -> Dict[str, Any]:
    """
...
def map_inp_to_config(params: Dict[str, Any], base_config: SimulationConfig) -> SimulationConfig:
    """
    ...
    sim_defaults.output_altitudes_km = [] # Default to empty, let specific mapping fill it
    sim_defaults.viewing_geometry = "custom" # Default to custom (no umu 1.0 enforcement)
    ...
    for k, v in params.items():
        if k not in processed_keys:
            if k == 'data_files_path': continue # handled by config paths
...
def generate_yaml(config: SimulationConfig, output_path: Path):
    ...
    data = {
        'paths': {
            ...
            'atmosphere_profile': str(config.paths.atmosphere_profile) if config.paths.atmosphere_profile else None,
            'solar_spectrum': str(config.paths.solar_spectrum) if config.paths.solar_spectrum else None,
...
def run_comparison(inp_file: Path, yaml_file: Path, config: SimulationConfig) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        resolved_params = resolve_paths(raw_params, inp_file.parent, config.paths.libradtran_data)
        ...
        if 'output_user' not in resolved_params and 'output' not in resolved_params:
             ...
             sim_ref.config.simulation_defaults.wavelength_nm = [] # Clear default wavelength
             
        dt = pd.to_datetime("2023-01-01 12:00:00").to_pydatetime()
        
        ref_out = sim_ref.run_simulation(dt=dt, latitude=0, longitude=0, parameter_overrides=resolved_params)
...
def main():
    ...
    for name in TARGET_EXAMPLES:
        ...
        try:
            ...
            resolved_params = resolve_paths(raw_params, examples_dir, base_config.paths.libradtran_data)
        except Exception:
            continue
    ...
    # Print Summary Table
    print("\n" + "="*80)
    print(f"{'EXAMPLE':<30} | {'STATUS':<10} | {'MAE':<10} | {'MRE':<10} | {'MAX_AE':<10}")
    print("-" * 80)
    for s in summary_stats:
        ...
        print(f"{s['name']:<30} | {s['status']:<10} | {mae_str:<10} | {mre_str:<10} | {max_ae_str:<10}")
    print("="*80)
