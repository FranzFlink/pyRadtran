...
def generate_uvspec_input_content(
    ...
    **kwargs
    ...
    try:
        ...
        # Handle molecule modifications with proper None checking
        mol_modify = getattr(sim_defaults, 'mol_modify', None)
        if mol_modify and isinstance(mol_modify, dict):
            # Handle ozone
            if 'O3' in mol_modify:
                o3_params = mol_modify['O3']
                lines.append(f"mol_modify O3 {o3_params['value']} {o3_params['unit']}")
            
            # Handle water vapor
            if 'H2O' in mol_modify:
                h2o_params = mol_modify['H2O']
                lines.append(f"mol_modify H2O {h2o_params['value']} {h2o_params['unit']}")
        
        # Fallback to individual parameters if mol_modify not used
        if not mol_modify:
            if hasattr(sim_defaults, 'ozone_du') and sim_defaults.ozone_du is not None:
                lines.append(f"mol_modify O3 {sim_defaults.ozone_du} DU")
            
            if hasattr(sim_defaults, 'h2o_mm') and sim_defaults.h2o_mm is not None:
        ...
        if hasattr(sim_defaults, 'output_columns') and sim_defaults.output_columns:
            ...
            output_columns = list(sim_defaults.output_columns)
            ...
            output_str = " ".join(output_columns)
